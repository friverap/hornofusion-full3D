...
**F2.14 — el líquido en contacto con el lecho gotea, nunca monta en el gas (2026-09-26).** Revisión de la literatura de lechos empacados: en los modelos de dos fluidos de lechos de goteo (revisión arXiv 2409.10674) el arrastre líquido–sólido es Ergun dividido por el holdup al cuadrado y el arrastre gas–líquido es un término de película aparte («Ergun-type f_gl cannot be justified in the dispersed regime»); en el modelo de cuatro fluidos del alto horno (Austin, Nogami y Yagi, ISIJ Int. 37 (1997) 458) el arrastre sólido–líquido de Kozeny–Carman se construye «to ensure the liquid velocity approaches the solid velocity as the liquid volume fraction goes to zero»; los holdups de coque (Fukutake, Sugiyama; ISIJ Int. 54 (2014) 820) dan goteo a mm/s–cm/s; AIAD reserva el régimen de gota a α_l < 0.3 dentro de gas continuo. Nuestra celda recién fundida bajo el arco (α_s 0.003–0.01, bajo el umbral 1e-2 de «lecho») se clasificaba freeboard y su líquido disperso tomaba el drift del jet (20 m/s) hacia la celda del lecho de abajo. F2.14 (`compute_liquid_drift`): toda celda con chatarra propia (α_s > `ALPHA_SOLID_RESID`) o que descansa sobre una celda del lecho (α_s(k−1) ≥ 1e-2) está en régimen de goteo: drift = `percolation_velocity` con α_s efectivo = max(propio, de abajo). Jueces de reinicio de v30: ventana de ráfagas 80–88.5 s **sin ningún episodio** (p_max 15.6 frente a 79 kPa; líquido máx. 3.8 frente a 20 m/s; fusión igual, 264 frente a 266 kg); ventana 110–120 s con F2.13: sin episodios (la única fila > 20 kPa es el instante del reinicio). Es la corrección consistente que F2.12-B no era: cambia la velocidad de la celda (transporte, energía y CFL ven lo mismo) en vez de acotar un flujo de cara.
