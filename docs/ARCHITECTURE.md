...
### Bug 19 — El Poisson no ve el cambio de composición: presión de MPa al formarse el baño (`mod_pressure_3d.f90`)

**Síntoma:** B1 v16, con todo lo anterior cerrado y una corrida por lo demás sana (masa exacta, clip 0, baño creciendo), la presión escala hasta la cota a partir de t ≈ 65 s y se queda allí. La correlación con el estado del horno es uno a uno: a t = 60 s no hay ninguna celda sin gas y |p| ≤ 250 Pa; a t = 65 s aparecen 8 bolsas sin gas y p_max = 77 kPa; a t = 70 s, 32 bolsas y 429 kPa; a **t = 75 s aparecen las dos primeras celdas de líquido puro (α_l = 1.000) y tienen 1.7 MPa**, con signo negativo (succión). Es decir: el blow-up empieza exactamente cuando el baño se forma.
**Causa:** el Poisson de mezcla impone `div(Σ α_q ρ_q u_q) = 0` sin el transitorio `∂ρ_m/∂t`. Mientras hay gas en todas las celdas, el término acústico del gas absorbe la discrepancia; en una celda de baño que se llena o drena —sin gas y con el líquido casi incompresible— la única forma de satisfacer divergencia nula es una presión de megapascales. El modelo, por construcción, no podía simular un baño real. (Los `U_LIQ_MAX`, `U_GAS_MAX` y `P_HYDRO_CAP` acotan el daño pero no la causa.)
**Fix:** el Poisson resuelve la continuidad de la mezcla **fluida**, así que su fuente es el transitorio de ésta menos lo que entra desde el sólido:

    ∂(α_l ρ_l + α_g ρ_g)/∂t + div(α_l ρ_l u_l + α_g ρ_g u_g) = ṁ_fusión
    ⇒  Su −= (V/Δt)·[(α_l ρ_l + α_g ρ_g) − (α_l ρ_l + α_g ρ_g)^n] − ṁ

con el ancla tomada al inicio del paso (`store_mixture_density`, antes de fusión y colapso) y la cota `COMP_SRC_CAP` referida a la masa **fluida** de la celda. Subsume el antiguo término de compresibilidad del gas (α_g fijo, ρ_g(T)) y añade lo que faltaba, el cambio de composición. Tres comprobaciones que fijan la formulación:
- **Fusión: exactamente neutra.** El `∂(α_l ρ_l)/∂t` que crea cancela con `ṁ`. Ahí está la explicación del intento histórico documentado en `mod_pressure_3d` («no reintroducir la fuente de masa de fusión: ganancia > 1, p×50/paso»): aquella versión metía *sólo* `ṁ`, sin su contrapartida, y fundir no crea volumen.
- **El sólido NO entra en ρ_f.** Primera versión de este fix: incluirlo (ρ_m con las cuatro fases) hace que el colapso de chatarra pida al *fluido* que reponga `δ·ρ_s` ≈ 3750 kg/m³, cuando físicamente sólo debe entrar el gas que llena el hueco, `δ·ρ_g` ≈ 0.1 kg/m³ — 4 órdenes de magnitud. El sólido no tiene flujo en el Poisson, así que tampoco puede tener transitorio en él.
- **La cota va con la masa fluida.** La primera versión la puso sobre ρ_m (con el sólido dentro), lo que la aflojó ~50 000× y desestabilizó hasta `cold_10step` (p a la cota en 10 pasos): `COMP_SRC_CAP` existe precisamente porque esta fuente es rígida.
