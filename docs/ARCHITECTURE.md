...
**B1 v21 (malla media, F2) — ALERT a t = 34–37 s y F2.4 (2026-09-24).** Sana hasta 33 s; después 8 filas con p > 300 kPa hasta la cota (2 MPa a 37.1 s) y gas en `U_GAS_MAX`, cada pico con una ráfaga de fusión y disipándose en décimas de segundo (`campaigns/b1_v21_f2_120s/VEREDICTO.md`). Mecanismo, **más temprano y distinto** que el de v19: celdas altas del lecho frío (k = 5, α_s 0.65–0.76) que se **llenan** de fundido percolado desde el charco de encima hasta α_g = 0.010 (`ALPHA_FLOW_CUTOFF`) → el gas deja de ser activo y la celda queda sellada por Ergun con +32 kPa (MPa entre snapshots): el "water packing" de los códigos de sistema. Causa numérica: en la cara charco|lecho el Rhie–Chow usaba la media **aritmética** de la movilidad `d = αV/aP` y de la velocidad de celda, así que una cara entre una celda libre (charco) y una bloqueada (Ergun, d ≈ 0) quedaba medio abierta y el charco vertía al lecho a la mitad de su movilidad. **Fix F2.4 (`mobility_face`)**: para el **líquido**, `d_f` armónico y velocidad y gradiente de cara ponderados por movilidad (`w_P = d_N/(d_P+d_N)`: el lado bloqueado manda, como conductancias en serie; MWIM). Sólo para el líquido: aplicado también al gas, la cara gas entre la celda de superficie (gas esclavizado por K, d ≈ 0) y la de encima quedaba cerrada, el Poisson dejaba de ver la velocidad vertical de esa celda y el gas se disparaba sin realimentación de presión (`bath_fill`: 300 m/s uniformes en la capa sobre la película). Bancos con F2.4: `bath_test` ferrostática 0.9 %, gas 21, fantasma 0.18; `bath_fill` gas 101, acero 1e-10; `bath_freeze` gas 14, clip 0.03 kg. Juez siguiente: reproductor grueso (`campaigns/b1_v22c_coarse_f24`).
